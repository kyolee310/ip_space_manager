...
sub print_output{
	my $str = shift @_;
	my $ts = print_time();
	my $outstr = "$ts [RESERVE_SUBNET_IP] [LOG]\t" . $str;
	print $outstr . "\n";
...
sub print_error{
	my $str = shift @_;
	my $ts = print_time();
	my $outstr = "$ts [RESERVE_SUBNET_IP] [ERROR]\t" . $str;
	print $outstr . "\n";
	system("echo \"$outstr\" >> $LOGFILE");
	print_log_space();
	exit(1);
...
if( @ARGV < 2 ){
	print_error("USAGE : ./reserve_subnet_ip.pl <OWNER> <SUBNET IPs>");
};
...
print_output("RESERVING SUBNET IPs: { $input_str }");

validate_input($input_str);

reserve_ips($owner, $input_str);

print_output("RESERVED SUBNET IPs: { $input_str }");
